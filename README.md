...
# Changes Made
**Overall**
- All instances of `charactertemplate` are removed.
- All instances of `charactertemplateNspecProjectile` are replaced with `projectile`.
...
- Renamed sprite files to be simpler.
   - e.g `character_body_circle` --> `circle`
- Updated the palette preview image.
- Removed a bunch of unused files.
- Moved sprite files into more reasonable folders.
   - `0_body pieces` is a dumb name.
...
- Set metadata object type to `Projectile` because it wasn't set before.
